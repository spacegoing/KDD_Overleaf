...
is a binary Markov random fields model which incorporates
...
detailed architecture are shown in Figure~\ref{fig:mrfrnn}.
...
represent those aspects as much as possible. Multi-task learning
...
information can be learned by high level task, we propose an
architecture that using trend and volatility tasks as our low
level tasks and price movement prediction (upward or downward) as
the high level task.
...
attention based LSTM~\cite{attention}. Specifically, bottom level
contains two separate DARNN modules. They are supervised by lower
level tasks which aim to predict future price as well as
volatility based upon the raw market price data. The key
difference among those modules is the loss function. At top
level, it is supervised by a high-level task that learns to use
representations extracted by two bottom modules as well as raw
...
For both DARNN modules at the low level, the input matrix is a
...
(y_1,y_2,\dots,y_T) \in \reals^T$. The aim of these two modules
are to predict target series $y_{t+p}$ in the next $p$ time
steps:
...
Besides time series classification, it can also be applied to
...
worse performance on all 4 metrics. In particular, performance of
