...
our back-propagation rules can be defined by taking derivative of the objective function \textit{w.r.t} $\bw^U$  defined in
...
\end{align}
%
where $W_{\!c}(\by_c) = \sum_{i \in c} w_i^c y_i$ with $w^c_i \geq
...
\begin{align}
  ...
  E^c(\by_c,\bz; \btheta^H) = \btheta^{H^T} \! \psi^H(\by_c,\bz)
\end{align}
...
\begin{align}
  ...
  (\mathbf{y}^*_k(\btheta^H),\mathbf{z}^*_k(\theta^H))=\argmin_{(\mathbf{y}
  \times \mathbf{z}) \in \mathcal{Y} \times \mathcal{Z}}
  \btheta^{H^T}\cdot\psi^H(\mathbf{y}_k,\mathbf{z}_k)
\end{align}
and
\begin{align}
...
\end{equation}
where $\btheta^H\in \reals^{2K-1}$ is the parameter vector in higher
...
\small
\setlength\tabcolsep{2pt}
\caption{Results: baselines and ablation studies. All models have a
...
\\                                                                        {\small (a) Ground-truth} & {\small (b) MMPL } & {\small (c) MMPL-MRFs} 
  ...
  \caption{\label{fig:consistency} Higher order consistency
    ...
    MMPL-MRFs on test set as inputs.}\vspace{-3mm}
\end{figure*}
...
\begin{acks}
  Li would like to thank Stephen Gould very much for explaining higer-order MRFs and thank Peerajak
  Witoonchart for the explanation of sub-gradient SSVMs algorithm. Li acknowledges data provision, MQD research infrastructure support and funding support from CMCRC. This work is also funded by China Hebei Provincial Department of Human Resources. Project ID: CL201723.
\end{acks}
